...
async def test_request_otp_happy_path(client, db_session):
    response = await client.post(
        "/api/v2/auth/request-otp", json={"email": "test@example.com"}
    )
...
async def test_request_otp_throttle(client):
    ...
    sixth = await client.post(
        "/api/v2/auth/request-otp", json={"email": "throttle@example.com"}
    )
...
async def test_request_otp_invalid_email(client):
    r = await client.post(
        "/api/v2/auth/request-otp", json={"email": "not-an-email"}
    )
