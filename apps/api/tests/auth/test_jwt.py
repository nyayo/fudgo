...
async def test_access_token_works_on_protected_endpoint(client, make_user, db_session):
    ...
    r = await client.get("/api/v2/auth/profile", headers={"Authorization": f"Bearer {token}"})
...
async def test_expired_access_token_rejected(client, make_user, db_session):
    ...
    await db_session.commit()
    decode_token(create_access_token(user.id))
    ...
    r = await client.get("/api/v2/auth/profile", headers={"Authorization": f"Bearer {expired}"})
...
async def test_refresh_rotates_and_revokes_old(client, db_session):
    ...
    r1 = await client.post("/api/v2/auth/refresh", json={"refresh": refresh})
    ...
    r3 = await client.post("/api/v2/auth/refresh", json={"refresh": new_pair["refresh"]})
...
async def test_logout_all_revokes_all_refresh(client, db_session):
    ...
    r3 = await client.get("/api/v2/auth/profile", headers={"Authorization": f"Bearer {access2}"})
...
async def test_wrong_signature_rejected(client, make_user, db_session):
    ...
    r = await client.get("/api/v2/auth/profile", headers={"Authorization": f"Bearer {bad}"})
    assert r.status_code == 401
