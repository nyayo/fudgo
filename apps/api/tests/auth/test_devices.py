...
async def test_register_device(client, make_user, db_session):
    ...
    with patch("app.auth.router.push_service.register", new=AsyncMock()) as mock_reg:
        r = await client.post(
            "/api/v2/auth/devices",
            headers=headers,
...
async def test_test_notification(client, make_user, db_session):
    ...
    await db_session.commit()
    headers = {"Authorization": f"Bearer {create_access_token(user.id)}"}
    with patch("app.auth.router.push_service.send", new=AsyncMock()) as mock_send:
...
async def test_delete_device(client, make_user, db_session):
    ...
    await db_session.commit()
    headers = {"Authorization": f"Bearer {create_access_token(user.id)}"}
    r = await client.delete(f"/api/v2/auth/devices/{device.id}", headers=headers)
    assert r.status_code == 200
